function OMEGA_Figure5A (outpath, figpath, rep, cfg)
...
% -------------------------------------------------------- %
% F5A.1. Plot of z-values for each frequency band and AAL region
% -------------------------------------------------------- %
...
propkz  = (propk-mean(propk,2))./std(propk,0,2);       
...
for roi = 1:Nroi
    z = propkzm(:,represvox(roi));
    for fb = 1:Nfb
        zfbands(roi,fb) = median(z(idf(fbands{fb})));       
    end
end
...
imagesc(zfbands)
caxis([-1 1.6])
colormap(flipud(gray));        % change colormap to gray (higher values are black)

textStrings = num2str(zfbands(:), '%0.1f');       
textStrings = strtrim(cellstr(textStrings));  
[x,y] = meshgrid(1:Nfb,1:Nroi); 
hStrings = text(x(:),y(:),textStrings(:),'HorizontalAlignment','center','FontSize',8);
% midValue = mean(get(gca, 'CLim'));                 % z > 0.3 is white  
midValue = 0.25;                                     % z > 0.3 is always white  
textColors = repmat(zfbands(:) > midValue, 1, 3);       % choose white or black for the text color of the strings depending on background colour
...
cd(figpath)
print('-dtiff','-r300',['Z_aal_fbands.tiff']);
